...
PLUGIN_PACKAGE_NAME="pulumi-resource-random-${VERSION}-$(go env GOOS)-$(go env GOARCH).tar.gz"
...
(cd "${ROOT}/provider" && go build \
   -ldflags "-X github.com/pulumi/pulumi-random/provider/v2/pkg/version.Version=${VERSION}" \
   -o "${WORK_PATH}/pulumi-resource-random${BIN_SUFFIX}" \
   "${ROOT}/cmd/pulumi-resource-random")
...
CREDS_JSON=$(aws sts assume-role \
                 ...
                 --role-session-name "upload-plugin-pulumi-resource-random" \
